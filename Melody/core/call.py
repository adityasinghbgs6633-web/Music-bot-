...
from Melody.utils.inline.play import stream_markup, suggestion_markup
...
# Standard FFmpeg parameters for smooth streaming in 2026
# Optimized for low-latency and reconnection on resource-constrained environments like Heroku
FFMPEG_PARAMS = (
    "-reconnect 1 -reconnect_at_eof 1 -reconnect_streamed 1 -reconnect_delay_max 5 "
    "-probesize 128 -analyzeduration 0 -fflags nobuffer -flags low_delay"
)
...
class Call(PyTgCalls):
        ...
        for session, client in zip(sessions, clients):
            if session:
                try:
                    ...
                    LOGGER(__name__).warning(f"Force stop: Error leaving call {chat_id}: {e}")
        try:
            await _clear_(chat_id)
        except Exception as e:
    ...
    async def force_stop_stream(self, chat_id: int):
        ...
        try:
            ...
            LOGGER(__name__).warning(f"Error popping from db for {chat_id}: {e}")
        await remove_active_video_chat(chat_id)
        await remove_active_chat(chat_id)
        try:
            await assistant.leave_call(chat_id)
    ...
    async def skip_stream(
        ...
        if video:
            stream = MediaStream(
                ...
                audio_parameters=audio_q,
                video_parameters=video_q,
                ffmpeg_parameters=FFMPEG_PARAMS if link.startswith("http") else None,
            )
        else:
            stream = MediaStream(
                link,
    ...
    async def change_stream(self, client, chat_id):
        ...
        try:
            ...
            if not check:
                await _clear_(chat_id)
                # Auto-suggestion feature when queue is empty
                if popped:
                    v_id = popped.get("vidid")
                    c_id = popped.get("chat_id")
                    t_id = popped.get("thread_id")
                    if v_id and v_id not in ["telegram", "soundcloud", "ytdlp"] and not v_id.startswith("http"):
                        try:
                            related = await YouTube.get_related(v_id)
                            if related and related.get("result"):
                                language = await get_lang(chat_id)
                                _ = get_string(language)
                                # Take the first related track
                                suggested_track = related["result"][0]
                                s_title = suggested_track["title"]
                                s_duration = suggested_track["duration"]
                                s_vidid = suggested_track["id"]
                                s_thumbnail = suggested_track["thumbnails"][0]["url"].split("?")[0]

                                buttons = suggestion_markup(_, s_vidid, app.id)
                                await app.send_photo(
                                    chat_id=c_id,
                                    photo=s_thumbnail,
                                    caption=_["stream_3"].format(s_title[:50], s_duration),
                                    reply_markup=InlineKeyboardMarkup(buttons),
                                    message_thread_id=t_id,
                                )
                        except Exception as e:
                            LOGGER(__name__).error(f"Error in auto-suggestion: {e}")

                return await client.leave_call(chat_id)
        except Exception as e:
            LOGGER(__name__).error(f"Error popping from db in change_stream for {chat_id}: {e}")
            try:
                await _clear_(chat_id)
                return await client.leave_call(chat_id)
            except Exception as e:
                LOGGER(__name__).warning(f"Error leaving call during change_stream for {chat_id}: {e}")
                return
        ...
        thread_id = next_track.get("thread_id")
        ...
        if "live_" in queued_file:
            ...
            try:
                ...
                LOGGER(__name__).error(f"Error playing live stream in {chat_id}: {e}")
                return await app.send_message(original_chat_id, text=_["call_6"])
            ...
            try:
                ...
                LOGGER(__name__).error(f"Error downloading vid_{video_id} in {chat_id}: {e}")
                return await mystic.edit_text(_["call_6"], disable_web_page_preview=True)
            ...
            if is_video:
                stream = MediaStream(
                    ...
                    audio_parameters=audio_q,
                    video_parameters=video_q,
                    ffmpeg_parameters=FFMPEG_PARAMS if file_path.startswith("http") else None,
                )
            else:
                stream = MediaStream(
                    file_path,
            ...
            try:
                ...
                LOGGER(__name__).error(f"Error playing vid_{video_id} in {chat_id}: {e}")
                return await app.send_message(original_chat_id, text=_["call_6"])
            ...
            if is_video:
                stream = MediaStream(
                    ...
                    audio_parameters=audio_q,
                    video_parameters=video_q,
                    ffmpeg_parameters=FFMPEG_PARAMS if video_id.startswith("http") else None,
                )
            else:
                stream = MediaStream(
                    video_id,
            ...
            try:
                ...
                LOGGER(__name__).error(f"Error playing index stream in {chat_id}: {e}")
                return await app.send_message(original_chat_id, text=_["call_6"])
            ...
            if is_video:
                stream = MediaStream(
                    ...
                    audio_parameters=audio_q,
                    video_parameters=video_q,
                    ffmpeg_parameters=FFMPEG_PARAMS if queued_file.startswith("http") else None,
                )
            else:
                stream = MediaStream(
                    queued_file,
            ...
            try:
                ...
                LOGGER(__name__).error(f"Error playing generic media stream in {chat_id}: {e}")
                return await app.send_message(original_chat_id, text=_["call_6"])
