...
async def delete_all_sudoers(client, message: Message, _):
    keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("Yes, Delete All", callback_data="confirm_delete_all_sudo", icon_custom_emoji_id="5996629301405945702"),
            InlineKeyboardButton("Cancel", callback_data="cancel_delete_all_sudo", icon_custom_emoji_id="5992179264315723303")
        ]
    ])
...
async def sudoers_list(client, message: Message, _):
    if not can_use_owner_commands(message.from_user.id) and message.from_user.id not in SUDOERS:
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("View Sudolist", callback_data="view_sudolist_unauthorized", icon_custom_emoji_id="5994560535918547027")]
        ])
