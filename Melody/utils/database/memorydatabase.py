...
topicsdb = mongodb.topics
logtopicdb = mongodb.logtopic
...
async def get_topic(chat_id: int) -> Optional[int]:
    if redis_client:
        top = redis_client.get(f"topic_{chat_id}")
        if top:
            t = int(top)
            return t if t != 0 else None

    top = topic.get(chat_id)
    if top is None:
        top_db = await topicsdb.find_one({"chat_id": chat_id})
        if not top_db:
            topic[chat_id] = 0
            if redis_client:
                redis_client.set(f"topic_{chat_id}", 0)
            return None
        topic[chat_id] = top_db["topic_id"]
        if redis_client:
            redis_client.set(f"topic_{chat_id}", top_db["topic_id"])
        return top_db["topic_id"]
    return top if top != 0 else None


async def set_topic(chat_id: int, topic_id: int):
    topic[chat_id] = topic_id
    if redis_client:
        redis_client.set(f"topic_{chat_id}", topic_id)
    await topicsdb.update_one(
        {"chat_id": chat_id}, {"$set": {"topic_id": topic_id}}, upsert=True
    )


async def delete_topic(chat_id: int):
    topic[chat_id] = 0
    if redis_client:
        redis_client.set(f"topic_{chat_id}", 0)
    await topicsdb.delete_one({"chat_id": chat_id})


async def get_log_topic(chat_id: int) -> Optional[int]:
    if redis_client:
        top = redis_client.get(f"log_topic_{chat_id}")
        if top:
            t = int(top)
            return t if t != 0 else None

    top = log_topic.get(chat_id)
    if top is None:
        top_db = await logtopicdb.find_one({"chat_id": chat_id})
        if not top_db:
            log_topic[chat_id] = 0
            if redis_client:
                redis_client.set(f"log_topic_{chat_id}", 0)
            return None
        log_topic[chat_id] = top_db["topic_id"]
        if redis_client:
            redis_client.set(f"log_topic_{chat_id}", top_db["topic_id"])
        return top_db["topic_id"]
    return top if top != 0 else None


async def set_log_topic(chat_id: int, topic_id: int):
    log_topic[chat_id] = topic_id
    if redis_client:
        redis_client.set(f"log_topic_{chat_id}", topic_id)
    await logtopicdb.update_one(
        {"chat_id": chat_id}, {"$set": {"topic_id": topic_id}}, upsert=True
    )


async def delete_log_topic(chat_id: int):
    log_topic[chat_id] = 0
    if redis_client:
        redis_client.set(f"log_topic_{chat_id}", 0)
    await logtopicdb.delete_one({"chat_id": chat_id})
...
async def get_vid_bit_name(chat_id: int) -> str:
    return video.get(str(chat_id), "SD_360p")
