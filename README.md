...
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=bisug&show_icons=true&theme=tokyonight&bg_color=00000000&border_color=00D9FF&hide_border=true" width="48%" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=bisug&layout=compact&theme=tokyonight&bg_color=00000000&border_color=00D9FF&hide_border=true" width="38%" />
</p>
...
Get Melody running in under 5 minutes:
...
# 5. Start the bot
uv run python -m Melody
```

> [!TIP]
> You need a `STRING_SESSION`, `BOT_TOKEN`, `API_ID`, `API_HASH`, and `MONGO_DB_URI` before the bot will start. See [Configuration](#-configuration) for details.

---

## ◈ Technical Overview

Melody is an optimized fork of the [**ShrutiMusic**](https://github.com/NoxxOP/ShrutiMusic) architecture, re-engineered for ultra-low latency and maximum uptime. This project was built with equal emphasis on technical correctness and visual polish.

### ◈ Core Architecture
- **Distributed Engine**: Leveraging a high-speed bypass API for resilient media delivery across all regions.
- **Asynchronous Core**: Built on the `Pyrogram` framework and `PyMongo` (MongoDB) for high-concurrency handling.
...
| Library | Role in Melody |
| :--- | :--- |
| [**PyMongo**](https://pymongo.readthedocs.io/) | Asynchronous driver for **MongoDB**, ensuring database operations never block the streaming loop. |
...
### ◈ Build Strategy
1.  **Stage 1 (Builder)**: Compiles Python dependencies and builds an isolated virtual environment using `uv` on `python:3.13-slim`.
...
# Sync dependencies (automatically creates virtualenv and installs from uv.lock)
uv sync
```
...
| `REDIS_URI` | No | Optional Redis string for faster cache. [See Guide](#redis-cache-guide) |
...
*   **Why it's Required:** These are the master keys that allow Melody to connect to Telegram's infrastructure. Without them, the bot cannot connect to the servers to stream music.
...
> [!NOTE]
> The `rediss://` prefix (double-s) indicates a TLS-encrypted connection, which Upstash requires. This is correct — it is not a typo.
...
| `START_IMG_URL` | No | Direct link to the image shown on /start. [See Guide](#image-hosting-guide) |

---

### ◈ Redis Cache Guide

To enable the Redis cache layer, you need a `rediss://` connection string from Upstash.

1.  Create a free account on [Upstash](https://upstash.com).
2.  Click **Create Database** and select **Redis**.
3.  Choose **Global** replication or a region closest to your hosting provider.
4.  Once created, open your database and copy the **Redis Connect String** from the **Connect** tab. It will look like `rediss://default:password@host:port`.

> [!NOTE]
> `rediss://` (double-s) means TLS-encrypted — this is intentional and required by Upstash, not a typo.

Paste this value as your `REDIS_URI` in `.env` or your platform's environment variable settings.
...
> [!IMPORTANT]
> Use a **dedicated, secondary YouTube account** for this — not your personal account. The exported cookies grant full account access, so treat them like a password.

#### Desktop (Chrome/Edge)
1. Install the **Get cookies.txt LOCALLY** extension.
2. Navigate to [YouTube](https://youtube.com) and sign into your dedicated secondary account.
3. Click the extension and export cookies in **Netscape** format.
4. Go to [BatBin](https://batbin.me) and paste your cookies.
5. Save and copy the **Raw URL** (e.g., `https://batbin.me/raw/...`).
6. After exporting, sign out of that account in your browser — the exported cookies remain valid until they expire or you revoke the session from account settings.

#### Mobile (Android/iOS)
1. Install [Mozilla Firefox](https://www.mozilla.org/firefox/) and the **cookies.txt** extension.
2. Navigate to [YouTube](https://youtube.com) and sign into your dedicated secondary account.
3. Use the extension to download cookies in **Netscape** format.
4. Paste to [BatBin](https://batbin.me) and get the **Raw URL**.
...
| Variable | Required | Description |
| :--- | :--- | :--- |
| `SUPPORT_CHANNEL` | No | Telegram channel username (e.g. `bisugMelody`). |
| `SUPPORT_GROUP` | No | Telegram group username (e.g. `bisugMusic`). |
...
To keep your bot running 24/7 on free hosting, use a cronjob service (like [Cron-job.org](https://cron-job.org/)) to ping your service URL every 1 minute.
...
6. **Save:** Your bot will now stay active.
...
Documentation was drafted with support from the following AI tools:

- [**Claude Sonnet 4.5**](https://www.anthropic.com/claude) — Technical documentation and logical restructuring.
- [**Gemini 2.0 Flash**](https://deepmind.google/technologies/gemini/) — Architectural analysis and rapid debugging insights.
- [**Google Jules**](https://jules.google.com) — Code review and refactoring assistance.
